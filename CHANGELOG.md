...
### Added
- **Hono adapter** — `honoAdapter(app, prisma, options)` for Cloudflare Workers, Bun, Deno, and Node
  ```ts
  import { Hono } from "hono";
  import { honoAdapter } from "omni-rest/hono";

  const app = new Hono();
  honoAdapter(app, prisma, { prefix: "/api" });
  export default app;
  ```
  - Full CRUD + `POST/PUT/DELETE /:model/bulk` routes
  - `hono` added as optional peer dependency
  - Exported from `omni-rest/hono`
