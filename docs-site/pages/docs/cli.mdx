...
<Tabs items={['generate:config', 'install:skills', 'generate (all)', 'generate:zod', 'generate:openapi']}>
  <Tabs.Tab>
    ```bash copy
  ...
  <Tabs.Tab>
    ...
    ```
    Copies the portable Omni Rest AI pack into another project. It installs the repo guide, the cross-project API playbook, and the tool-specific instruction files for Codex, Cursor, Claude, and Kiro.

    <Callout type="info">
      Use `--overwrite` when you want to refresh an existing pack without editing files by hand.
    </Callout>
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
