...
<Cards>
  <Card icon={<CheckCircle2 color="#22c55e" />} title="Recently Shipped" href="#recently-shipped">
    ?search= global search, soft delete (deletedAt / isActive), ?fields= alias for ?select=
  </Card>
  <Card icon={<AlertTriangle color="#ef4444" />} title="Good First Issues" href="#good-first-issues">
    Sort by relation count (?sort=_count.posts:desc), response envelope option, Express error handler middleware
  </Card>
  <Card icon={<Lightbulb color="#eab308" />} title="Intermediate" href="#intermediate">
    Nested relation filtering, bulk create/update/delete, field-level permissions, rate limit hook
  </Card>
  <Card icon={<GitPullRequest color="#a855f7" />} title="Advanced" href="#advanced">
    Cursor pagination, aggregation endpoints, SSE subscriptions, plugin architecture
  </Card>
</Cards>
