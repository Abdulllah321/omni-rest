...
| `id` | `string \| null` | Record ID for single-record operations |
...
| Option | Effect |
|--------|--------|
| `hidden` | Removed from all responses (GET single, GET list, POST/PUT response) |
| `readOnly` | Stripped from write bodies before reaching Prisma |
| `writeOnly` | Removed from all GET responses; accepted in write bodies |
