...
/**
 * Generates use[Model].ts TanStack Query hooks.
 * No @prisma/client dependency — uses generic Record types instead.
 */
export function generateHookFile(
  ...
  lines.push(``);
  // Generic type instead of @prisma/client
  lines.push(`type ${Model} = Record<string, any>;`);
  lines.push(`type ${Model}CreateInput = Record<string, any>;`);
  lines.push(`type ${Model}UpdateInput = Record<string, any>;`);
  lines.push(``);
  ...
  lines.push(`    mutationFn: (data: ${Model}CreateInput) =>`);
  ...
  lines.push(`  const queryClient = useQueryClient();`);
  lines.push(`  return useMutation({`);
  lines.push(`    mutationFn: ({ id, data }: { id: string; data: ${Model}UpdateInput }) =>`);
  ...
  lines.push(`      fetch(\`\${BASE_URL}/${routeName}/\${id}\`, { method: "DELETE" }).then((r) => r.json()),`);
  if (!noOptimistic) {
